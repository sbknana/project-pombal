...
```
You: "Why did task 142 fail three times before succeeding?"

Claude: Task 142 history:

  Attempt 1: Developer used wrong ORM syntax for Prisma batch upsert.
             Tester caught it — 2 of 5 tests failing.
  Attempt 2: Fixed the upsert but missed the transaction wrapper.
             Integration test timed out.
  Attempt 3: Added transaction, but forgot to handle the rollback
             on partial failure. Tester caught edge case.
  Attempt 4: All fixes applied. 5/5 tests passing.

  ForgeSmith extracted a lesson from this: "Always wrap Prisma
  batch operations in $transaction with explicit rollback handling."
  That lesson has been injected into 4 subsequent developer runs.
```
...
- **Checkpoint/resume** — Interrupted tasks pick up where they left off
- **Adaptive complexity** — Turn budgets scale with task difficulty
...
You also need access to an LLM provider (Anthropic API key, Claude Pro/Max, or local models via Ollama).
...
See [ORCHESTRATOR.md](docs/ORCHESTRATOR.md) for manual installation and CLI usage.
...
Manual mode works and is fully documented in [ORCHESTRATOR.md](docs/ORCHESTRATOR.md). But for most workflows, Coordinator Mode is faster, easier, and produces better results.
...
| Doc | What's in it |
|-----|-------------|
| [Capabilities](docs/CAPABILITIES.md) | Architecture deep dive, ForgeSmith, security pipeline, benchmarks |
| [Orchestrator](docs/ORCHESTRATOR.md) | CLI commands, flags, manual setup, advanced usage |
...
| [Custom Agents](docs/CUSTOM_AGENTS.md) | How to create your own agent roles |
